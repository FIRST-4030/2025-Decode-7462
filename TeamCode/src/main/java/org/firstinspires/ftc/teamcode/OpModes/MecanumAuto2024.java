...
public class MecanumAuto2024 extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (logHPZside) { sampleSideLog = new LogFile(LogFile.FileType.Action, "HPZ", "csv" ); }

        if (logGoalSide) { specimenSideLog = new LogFile(LogFile.FileType.Action, "Goal", "csv" ); }
        ...
        if(!side) {
            ...
            stepPose = new Pose2dWrapper(15.9, -55.75, 1.5708);
            depositPose = new Pose2dWrapper(-60.5, -48, Math.toRadians(50));
            collectionPose = new Pose2dWrapper(-49, -35, 1.5708);
            collectionPose2 = new Pose2dWrapper(-58.5, -35, 1.5708);
            collectionPose3 = new Pose2dWrapper(-61.5, -30.25, Math.toRadians(125));
            preParkPose = new Pose2dWrapper(-42, -8.5, 0);
            parkPose = new Pose2dWrapper(-23.5, -7, 0);
        }
        ...
        if(!side) {
            if(!experimental){
                ///Cycle 1
                if (logGoalSide) { specimenSideLog.logAction( startPose.toPose2d(), "Cycle 1" ); }
                ...
                if (logGoalSide) { specimenSideLog.logAction( depositPose.toPose2d() ); }
                ...
                if (logGoalSide) { specimenSideLog.logAction( depositPose.toPose2d(), "Cycle 2" ); }
                Actions.runBlocking(
                ...
                if (logGoalSide) { specimenSideLog.logAction( brickPush2.toPose2d() ); }
                sleep(350);
                ...
                Actions.runBlocking(experimentalPreScore());
                sleep(100);
                if (logGoalSide) { specimenSideLog.logAction( brickPush2.toPose2d(), "Deposit 2" ); }
                Actions.runBlocking(
                        drive.actionBuilder(brickPush2.toPose2d())
                                .strafeTo(depositPose2.toPose2d().position)
                                .build()
                );
                if (logGoalSide) { specimenSideLog.logAction( depositPose2.toPose2d(), "" ); }
                Actions.runBlocking(
                        extendScore()
                );
                sleep(400);
                ...
                Actions.runBlocking(experimentalCollectionPrep());
                ///Cycle 3
                if (logGoalSide) { specimenSideLog.logAction( depositPose2.toPose2d(), "Cycle 3" ); }
                Actions.runBlocking(
                        drive.actionBuilder(depositPose2.toPose2d())
                                .strafeToLinearHeading(collectionPose.toPose2d().position, collectionPose.heading)
                                .build()
                );
                if (logGoalSide) { specimenSideLog.logAction( collectionPose.toPose2d() ); }
                Actions.runBlocking(experimentalExtend());
                sleep(275);
                Actions.runBlocking(collect());
                sleep(300);
                Actions.runBlocking(experimentalPreScore());
                if (logGoalSide) { specimenSideLog.logAction( collectionPose.toPose2d(), "Deposit 3" ); }
                Actions.runBlocking(
                        drive.actionBuilder(collectionPose.toPose2d())
                                .strafeTo(depositPose3.toPose2d().position)
                                .build()
                );
                if (logGoalSide) { specimenSideLog.logAction( depositPose3.toPose2d() ); }
                Actions.runBlocking(extendScore());
                sleep(400);
                Actions.runBlocking(openClaw());
                sleep(250);
                ///Cycle 4
                Actions.runBlocking(experimentalCollectionPrep());
                if (logGoalSide) { specimenSideLog.logAction( depositPose3.toPose2d(), "Cycle 4" ); }
                Actions.runBlocking(
                ...
                Actions.runBlocking(experimentalPreScore());
                sleep(100);
                if (logGoalSide) { specimenSideLog.logAction( collectionPose.toPose2d(), "Deposit 4" ); }
                Actions.runBlocking(
                        drive.actionBuilder(collectionPose.toPose2d())
                                .strafeTo(depositPose4.toPose2d().position)
                                .build()
                );
                if (logGoalSide) { specimenSideLog.logAction( depositPose4.toPose2d() ); }
                Actions.runBlocking(extendScore());
                sleep(400);
                Actions.runBlocking(openClaw());
                sleep(300);
                Actions.runBlocking(stow());
                if (logHPZside) { sampleSideLog.logAction( depositPose4.toPose2d(), "Park" ); }
                Actions.runBlocking(
                        drive.actionBuilder(depositPose4.toPose2d())
                                .strafeTo(parkPose.toPose2d().position)
                                .build()
                );
                if (logGoalSide) { specimenSideLog.logAction( parkPose.toPose2d() ); }
            } else {
                ...
                Actions.runBlocking(floorPickUpPrep1());
                Actions.runBlocking(
                        drive.actionBuilder(new Pose2dWrapper(anchorPose2.toPose2d().position.x, anchorPose2.toPose2d().position.y, Math.toRadians(295)).toPose2d())
            ...
            if (experimental) {
                ...
                if (logHPZside) { sampleSideLog.logAction( startPose.toPose2d(), "Cycle 1" ); }
                Actions.runBlocking(
                        drive.actionBuilder(startPose.toPose2d())
                                .strafeToLinearHeading(depositPose.toPose2d().position, depositPose.heading)
                                .build()
                );
                if (logHPZside) { sampleSideLog.logAction( depositPose.toPose2d() ); }
                sleep(1000);
                Actions.runBlocking(openClaw2());
                ///Cycle 2
                if (logHPZside) { sampleSideLog.logAction( depositPose.toPose2d(), "Cycle 2" ); }
                sleep(800);
                ...
                if (logHPZside) { sampleSideLog.logAction( collectionPose.toPose2d() ); }
                sleep(800);
                Actions.runBlocking(samplePickUp());
                sleep(300);
                Actions.runBlocking(collect());
                sleep(250);
                Actions.runBlocking(highBucketPrep());
                if (logHPZside) { sampleSideLog.logAction( collectionPose.toPose2d(), "Deposit" ); }
                Actions.runBlocking(
                        drive.actionBuilder(collectionPose.toPose2d())
                                .strafeToLinearHeading(depositPose.toPose2d().position, depositPose.heading)
                                .build()
                ...
                Actions.runBlocking(openClaw2());
                sleep(800);
                if (logHPZside) { sampleSideLog.logAction( depositPose.toPose2d(), "Collection 2" ); }
                Actions.runBlocking(floorPickUpPrep());
                Actions.runBlocking(
                        drive.actionBuilder(depositPose.toPose2d())
                ...
                if (logHPZside) { sampleSideLog.logAction( collectionPose2.toPose2d() ); }
                sleep(800);
                Actions.runBlocking(samplePickUp());
                sleep(300);
                Actions.runBlocking(collect());
                sleep(250);
                Actions.runBlocking(highBucketPrep());
                if (logHPZside) { sampleSideLog.logAction( collectionPose2.toPose2d(), "Deposit" ); }
                Actions.runBlocking(
                ...
                if (logHPZside) { sampleSideLog.logAction( depositPose.toPose2d(), "Collection 3" ); }
                Actions.runBlocking(
                        drive.actionBuilder(depositPose.toPose2d())
                                .strafeToLinearHeading(collectionPose3.toPose2d().position, collectionPose3.toPose2d().heading)
                                .build()
                );
                if (logHPZside) { sampleSideLog.logAction( collectionPose3.toPose2d() ); }
                sleep(750);
                ...
                sleep(500);
                Actions.runBlocking(highBucketPrep());
                if (logHPZside) { sampleSideLog.logAction( collectionPose3.toPose2d(), "Deposit" ); }
                Actions.runBlocking(
                ...
                if (logHPZside) { sampleSideLog.logAction( depositPose.toPose2d(), "Pre-Park" ); }
                Actions.runBlocking(
                        drive.actionBuilder(depositPose.toPose2d())
                                .strafeToLinearHeading(preParkPose.toPose2d().position, preParkPose.toPose2d().heading)
                                .build()
                );
                if (logHPZside) { sampleSideLog.logAction( preParkPose.toPose2d() ); }
                if (logHPZside) { sampleSideLog.logAction( preParkPose.toPose2d(), "Park" ); }
                Actions.runBlocking(
                        drive.actionBuilder(preParkPose.toPose2d())
                                .strafeToLinearHeading(parkPose.toPose2d().position, preParkPose.toPose2d().heading)
                                .build()
                );
                if (logHPZside) { sampleSideLog.logAction( parkPose.toPose2d() ); }
                sleep(1500);
            } else {
                ...
                ///Cycle 2
                Actions.runBlocking(floorPickUpPrep());
                if (logHPZside) { sampleSideLog.logAction( depositPose.toPose2d(), "Sample 5" ); }
                Actions.runBlocking(
                        drive.actionBuilder(depositPose.toPose2d())
                                .strafeToLinearHeading(collectionPose.toPose2d().position, collectionPose.heading)
                                .build()
